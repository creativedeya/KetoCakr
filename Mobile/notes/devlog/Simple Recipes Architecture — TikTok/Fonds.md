...
componentName: {
  fontSize: 12,  // ← REDUCED from 14 (was categoryLabel at 17)
  fontWeight: '600',
...
stepsCount: {
  fontSize: 11,  // ← REDUCED from 12 to match role
  color: Colors.text.secondary,
...
After:
  [🍰 image] Блат Сахер (12px name)     8 стъпки
             Блат (role, 11px)           (on right, 11px)
```

Font Sizes:
- Component name: 12px (was 17px for categoryLabel)
- Component role: 11px
- Steps count: 11px
