...
Add THIS HELPER FUNCTION before the stepsForRecipe loop:
...
### Step 4: Add readyRecipeIngredientsData to dependencies
**File:** `Mobile/app/recipe-detail/[id].tsx`

**Location:** In the useMemo dependency array (closing bracket of useMemo)

Make sure `readyRecipeIngredientsData` is included:
...
const transformedData = useMemo(() => {
  ...
  baseRecipes,
  stepsData,
  readyRecipeIngredientsData,  // ← ADD THIS!
  language,
  recipeType,
  // ... other dependencies ...
]);
...
| File | Changes |
|------|---------|
| `Mobile/app/recipe-detail/[id].tsx` | Add query (Step 1) + helper function (Step 2) + transform logic (Step 3) + dependencies (Step 4) |
